...
# mongo_version=2.6.8
# container=gianpaj/boot2docker_2.6.8
container=gianpaj/boot2docker_3.0.0
...
# (equivalet to)
# docker run --name rs1_srv1 -P -d mongo mongod --noprealloc --smallfiles --replSet rs1 --keyFile /opt/keyfile
...
# docker run --name mongos -P -p 27017:27017 -d "$container" mongos --configdb "$CFG1":27017,"$CFG2":27017,"$CFG3":27017 --keyFile /opt/keyfile > /dev/null 2>&1
docker run --name mongos -P -p 27017:27017 -d "$container" mongos --configdb "$CFG1":27017,"$CFG2":27017,"$CFG3":27017 --keyFile /opt/keyfile --setParameter userCacheInvalidationIntervalSecs=30

# Wait for mongos node to start
sleep 5

# add admin user
echo "Adding admin user in admin db..."
docker exec -it mongos mongo admin --quiet --eval "printjson( db.createUser({ user: 'admin', pwd: 'password', roles: [ 'root' ]}) )"
...
docker exec -it mongos mongo admin --quiet --eval "$ADDSHARDS"
...
read -r -d '' ENABLESHARDING <<- EOM
	db.auth('admin', 'password');
	printjson( db.getSisterDB('test').test.insert({a: 1}) );
	printjson( db.getSisterDB('test').test.ensureIndex( { _id: "hashed" } ) );
	printjson( db.adminCommand( { enableSharding: "test" } ) );
	printjson( sh.shardCollection( "test.test", { _id: "hashed" } ) );
EOM

docker exec -it mongos mongo admin --quiet --eval "$ENABLESHARDING"

# Wait for sharding to be enabled
sleep 5

echo "increase log level"
docker exec -it mongos mongo admin --quiet --eval "db.auth('admin', 'password'); printjson( db.adminCommand( { setParameter: 1, logLevel: 2 } ) );"

echo "#####################################"
echo "MongoDB Cluster is now ready to use"
echo "Connect to cluster via docker:"
echo "$ docker exec -it mongos mongo --authenticationDatabase admin -u admin -p password"
echo ""
echo "Connect to cluster via OS X:"
echo "$ mongo --host $(boot2docker ip) --authenticationDatabase admin -u admin -p password"
